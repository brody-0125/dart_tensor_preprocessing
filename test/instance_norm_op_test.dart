...
void main() {
  group('InstanceNormOp', () {
    test('normalizes 3D tensor per channel', () {
      ...
      final data =
          Float32List.fromList([0, 2, 4, 6, 1, 3, 5, 7].map((e) => e.toDouble()).toList());
      ...
      final ch0Mean =
          ch0Values.reduce((a, b) => a + b) / ch0Values.length;
      ...
      final ch1Mean =
          ch1Values.reduce((a, b) => a + b) / ch1Values.length;
    ...
    test('normalizes 4D tensor per sample per channel', () {
      ...
      final b0c0Mean =
          b0c0Values.reduce((a, b) => a + b) / 4;
      expect(b0c0Mean, closeTo(0.0, 1e-5));

      // Batch 1, Channel 1: indices 12-15
      final b1c1Values = [12, 13, 14, 15].map((i) => result.storage.getAsDouble(i));
      final b1c1Mean =
          b1c1Values.reduce((a, b) => a + b) / 4;
      expect(b1c1Mean, closeTo(0.0, 1e-5));
    });

    test('applies affine transformation with weight and bias', () {
      final data = Float32List.fromList([0, 2, 4, 6, 1, 3, 5, 7].map((e) => e.toDouble()).toList());
      final tensor = TensorBuffer.fromFloat32List(data, [2, 2, 2]);
    ...
    test('works with Float64 dtype', () {
      final data = Float64List.fromList([0, 2, 4, 6, 1, 3, 5, 7].map((e) => e.toDouble()).toList());
