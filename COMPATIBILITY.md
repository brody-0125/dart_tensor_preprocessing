...
| `PositionalEncodingOp` | [lib/src/ops/positional_encoding_op.dart](lib/src/ops/positional_encoding_op.dart) | `positional-*`: independent torch sin/cos recipe, odd/even dimensions, rank 2/3/4, all ten dtypes, offset/in-place/strided views. Double addition then native storage conversion for integer inputs (large integers may round); additive positional encoding, not RoPE or learned Embedding. Finite positive base and rank/dimension/maxLen validated. |
...
| `RandomCropOp` | [lib/src/ops/augmentation_op.dart](lib/src/ops/augmentation_op.dart) | `random-crop-*`: all ten dtypes, exact int64/uint64, CHW/NCHW, offsets/strides, full/partial crops; recorded Dart seed-41 coordinates with independent torch slices, invalid rank/size tests |
| `RandomErasingOp` | [lib/src/ops/random_erasing_op.dart](lib/src/ops/random_erasing_op.dart) | `erase-*`: all ten dtypes, batch/view/in-place, skip/full/partial/impossible rectangles. Seed 41 placement recorded independently; torch assignment supplies values. Native tests verify uniform random fills and integer truncation. Unlike torchvision: independent per-image draws, rounded area, full-image rectangles, uniform rather than normal random fill; seeds are not interchangeable. |
